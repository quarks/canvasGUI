/**
 * This sketch is used to provide a visual editor to see how to use the grid
 * layout functionality of canvasGUI.
 * 
 * You show read the README file for more information about the grid layout 
 * and how to use it.
 */
const X = 40, Y = 30, W = 560, H = 160;
const NONE = 0, COL = 1, ROW = 2, CELL = 3;
let gui, bdrs = [], gridCode = [];
...
function setup() {
    let p5canvas = createCanvas(W + 2 * X, 800);
    is3d = p5canvas.GL;
    console.log(`Version ${VERSION}   using ${is3d ? 'WEBGL' : 'P2D'}`);
    resetInfo();
    gui = createGUI('demoGYU', p5canvas);
...
function drawCode() {
    push();
    textFont('Courier New'); textSize(18); textAlign(LEFT, TOP);
    let codeY = Y + H + 230, codeW = W - 20, codeH = height - codeY - 20;
    let la = 20, tab = 2 * textWidth(' '), gridLine = '';
    translate(X, codeY);
    noStroke(); fill(0); rect(0, 0, W, codeH); fill(0, 255, 0);
    for (let i = 0; i < gridCode.length; i++) {
        let left = 10 + gridCode[i][0] * tab, gridLine = gridCode[i][1];
        gridLine.charAt(0) == '/' ? fill(0, 255, 0) : fill(255, 255, 0);
        text(gridLine, left, 10 + i * la);
    }
    pop();
}
...
function makeCode() {
    gridCode = [];
    gridCode.push([0, `// Create the 5x4 grid layout shown above`]);
    gridCode.push([0, `gui.grid( ${X}, ${Y}, ${W}, ${H} )`]);
    if (ix != 2 || iy != 2) gridCode.push([1, `.insets( ${ix}, ${iy} )`]);
    gridCode.push([1, `  .cols( ${c$} )`]);
    gridCode.push([1, `  .rows( ${r$} );`]);
    gridCode.push([0, '']);
    gridCode.push([0, `// Get the position and size (pixels) of the`]);
    gridCode.push([0, `// green area and use this info when creating`]);
    gridCode.push([0, `// the gui control:  x  y  w  h`]);
    gridCode.push([0, `let area = grid.cell(${px}, ${py}, ${pw}, ${ph});`])
    gridCode.push([0, `console.log(...area);`]);
    let [x, y, w, h] = grid.cell(px, py, pw, ph);
    gridCode.push([0, `// Output: ${x} ${y} ${w} ${h} `]);
    gridCode.push([0, '']);
    gridCode.push([0, `// To create a label in this area use`]);
    gridCode.push([0, `gui.label('uid', ...grid.cell(${px}, ${py}, ${pw}, ${ph}))`]);
    gridCode.push([1, `  .text('Hello World'};`]);
    gridCode.push([0, `// 'uid' is a unique identifier for the control`]);
}
