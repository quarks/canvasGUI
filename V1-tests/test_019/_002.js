...
function setup() {
    ...
    gui.slider('sdr_e', 120, py1, width - 240, 30).scheme('red')
        ...
        .setAction((info) => { });
    gui.slider('sdr_s', 60, py1, height - 200, 30).scheme('blue')
        .orient('south')
        .opaque() // make the control background opaque
        .ticks(2, 10) // Make major and minor ticks
        .limits(-0.5, 0.5) // Set the min and max permitted value 
        .setAction((info) => { });
    gui.slider('sdr_n', 520, py1, height - 200, 30).scheme('green')
        .orient('north')
        .opaque() // make the control background opaque
        .ticks(2, 10) // Make major and minor ticks
        .limits(-0.5, 0.5) // Set the min and max permitted value 
        .setAction((info) => { });
    gui.slider('sdr_w', 120, py1 + 280, width - 240, 30).scheme('purple')
        .orient('west')
        .opaque() // make the control background opaque
        .ticks(2, 10) // Make major and minor ticks
        .limits(-0.5, 0.5) // Set the min and max permitted value 
