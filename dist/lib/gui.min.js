...
this._target.addEventListener("keyup",a=>{this._handleKeyEvents(a);return!1}),this._keyEventsEnabled=!0)}_addTouchEventHandlers(){if(!this._touchEventsEnabled){let a=this._canvas;a.addEventListener("touchstart",b=>{this._handleTouchEvents(b)});a.addEventListener("touchend",b=>{this._handleTouchEvents(b)});a.addEventListener("touchcancel",b=>{this._handleTouchEvents(b)});a.addEventListener("touchmove",b=>{this._handleTouchEvents(b)});this._touchEventsEnabled=!0}}_handleTouchEvents(a){if(this._eventsAllowed&&
this._enabled&&this.isVisible()){let b;for(let c of this._ctrls)if(b=void 0,c.isActive()){b=c;c._handleTouch(a);break}if(void 0==b)for(let c of this._ctrls)c.isEnabled()&&c.isVisible()&&c._handleTouch(a)}}_handleFocusEvents(a){switch(a.type){case "focusout":for(let b of this._ctrls)b.isActive()&&(b._deactivate?.(),b.validate?.())}}_handleKeyEvents(a){if(this._eventsAllowed&&this._enabled&&this.isVisible())for(let b of this._ctrls)if(b.isActive()){b._handleKey(a);break}return!1}_handleMouseEvents(a){if(this._eventsAllowed&&
this._enabled&&this.isVisible()){let b;for(let c of this._ctrls)if(c.isActive()){b=c;c._handleMouse(a);break}if(void 0==b)for(let c of this._ctrls)c.isEnabled()&&c.isVisible()&&c._handleMouse(a)}return!1}_selectDrawMethod(){this.draw=(this._is3D=void 0!=this._renderer.GL&&null!=this._renderer.GL)?this._drawControlsWEBGL:this._drawControlsP2D}$(a){return"string"===typeof a?this._controls.get(a):a}addControl(a){console.assert(!this._controls.has(a.name()),`Control '${a.name()}' already exists and will be replaced.`);
this._controls.set(a.name(),a);this._ctrls=[...this._controls.values()];this._ctrls.sort((b,c)=>b.z()-c.z());return a}listControls(){console.log("List of controls");for(let a of this._ctrls)console.log(a.name());console.log("--------------------------------------------------------------")}getOptionGroup(a){this._optionGroups.has(a)||this._optionGroups.set(a,new CvsOptionGroup(a));return this._optionGroups.get(a)}textSize(a){if(!Number.isFinite(a))return this._textSize;this._textSize=a;this._controls.forEach(b=>
{b.invalidateBuffer()});return this}tipTextSize(a){return a?(this._tipTextSize=a,this):this._tipTextSize}canvasWidth(){return this._renderer.width}canvasHeight(){return this._renderer.height}corners(a){Array.isArray(a)&&4==a.length&&(this._corners=[...a]);return[...this._corners]}context(){return this._renderer}is3D(){return this._is3D}_closePanes(){for(let a of this._panesEast)a.close();for(let a of this._panesWest)a.close();for(let a of this._panesSouth)a.close();for(let a of this._panesNorth)a.close()}hidePanes(){this._closePanes();
for(let a of this._panesEast)a.hide();for(let a of this._panesWest)a.hide();for(let a of this._panesSouth)a.hide();for(let a of this._panesNorth)a.hide()}showPanes(){for(let a of this._panesEast)a.show();for(let a of this._panesWest)a.show();for(let a of this._panesSouth)a.show();for(let a of this._panesNorth)a.show()}validateTabsEast(){let a=this._panesEast,b=a.length,c=2*(b-1);a.forEach(e=>c+=e.tab()._w);let d=(this.canvasHeight()-c)/2;for(let e=0;e<b;e++){let f=a[e].tab(),g=-f._h,h=d;d+=f._w+2;
f._x=g;f._y=h}}validateTabsWest(){let a=this._panesWest,b=a.length,c=2*(b-1);a.forEach(f=>c+=f.tab()._w);let d=(this.canvasHeight()-c)/2;for(let f=0;f<b;f++){var e=a[f];let g=e.tab();e=e.depth();let h=d;d+=g._w+2;g._x=e;g._y=h}}validateTabsSouth(){let a=this._panesSouth,b=a.length,c=2*(b-1);a.forEach(e=>c+=e.tab()._w);let d=(this.canvasWidth()-c)/2;for(let e=0;e<b;e++){let f=a[e].tab(),g=d,h=-f._h;d+=f._w+2;f._x=g;f._y=h}}validateTabsNorth(){let a=this._panesNorth,b=a.length,c=2*(b-1);a.forEach(f=>
c+=f.tab()._w);let d=(this.canvasWidth()-c)/2;for(let f=0;f<b;f++){var e=a[f];let g=e.tab(),h=d;e=e.depth();d+=g._w+2;g._x=h;g._y=e}}_initColorSchemes(){this._schemes=[];this._schemes.blue=new BlueScheme;this._schemes.green=new GreenScheme;this._schemes.red=new RedScheme;this._schemes.cyan=new CyanScheme;this._schemes.yellow=new YellowScheme;this._schemes.purple=new PurpleScheme;this._schemes.orange=new OrangeScheme;this._schemes.light=new LightScheme;this._schemes.dark=new DarkScheme;this._scheme=
this._schemes.blue}scheme(a){if(!a)return this._scheme;this._schemes[a]?(this._scheme=this._schemes[a],this._controls.forEach(b=>{b.scheme()||b.invalidateBuffer()})):console.error(`'${a}' is not a valid color scheme`);return this}_getScheme(a){if(a&&this._schemes[a])return Object.assign({},this._schemes[a]);console.warn(`Unable to retrieve color scheme '${a}'`)}addScheme(a,b){"string"!==typeof a||b||(this._schemes[a]?console.error(`Cannot add scheme '${a}' because it already exists.'`):this._schemes[a]=
b);return this}draw(){}_drawControlsP2D(){if(this._visible){this._p.push();for(let a of this._ctrls)a.getParent()||a._renderP2D();this._p.pop()}}_drawControlsWEBGL(){if(this._visible){this._p.push();let a=this._renderer,b=a.drawingContext,c=a.width,d=a.height,e=Number.MAX_VALUE;b.flush();let f=a.uMVMatrix.copy(),g=a.uPMatrix.copy();b.disable(b.DEPTH_TEST);a.resetMatrix();this._getCamera().ortho(0,c,-d,0,-e,e);for(let h of this._ctrls)h.getParent()||h._renderWEBGL();b.flush();a.uMVMatrix.set(f);a.uPMatrix.set(g);
b.enable(b.DEPTH_TEST);this._p.pop()}}static announce(){GUI._announced||(console.log("================================================"),console.log(`  canvasGUI (${CANVAS_GUI_VERSION})   \u00A9 2025 Peter Lager`),console.log("================================================"),GUI._announced=!0)}static get(a,b=p5.instance){GUI.announce();if(GUI._guis.has(a))return GUI._guis.get(a);b=new GUI(a,b);GUI._guis.set(a,b);return b}static getNamed(a,b,c=p5.instance){GUI.announce();if("string"===typeof a&&
0<a.length){if(GUI._guis.has(a))return GUI._guis.get(a);b=new GUI(b,c);b._name=a;GUI._guis.set(a,b);return b}}static find(a){return GUI._guis.get(a)}}GUI._guis=new Map;GUI._announced=!1;
...
class PurpleScheme extends BaseScheme{constructor(){super();this._color(300);this._grey("light")}}class LightScheme extends BaseScheme{constructor(){super();this._mono(254,0);this._grey("light")}}class DarkScheme extends BaseScheme{constructor(){super();this._mono(0,254);this._grey("dark");this._whiteTints()}}
...
let [e,f,g,h]=this._orientation.xy(d.clientX-c.left-b.x,d.clientY-c.top-b.y,this._w,this._h);this._pover=this._over;this._over=this._whereOver(e,f,5);this._bufferInvalid=this._bufferInvalid||this._pover!=this._over;this._tooltip&&this._tooltip._updateState(this,this._pover,this._over);this._processEvent(a,e,f,g,h)}};
...
this.invalidateBuffer())}}_updateControlVisual(){let a=this._buffer,b=this._scheme||this._gui.scheme(),c=a.width-20;var d=b.C_3,e=b.G_7,f=b.G_3,g=b.G_1;const h=b.C_9,k=b.C_6;a.push();a.clear();this._opaque&&(a.noStroke(),a.fill(d),a.rect(0,0,this._w,this._h,this._c[0],this._c[1],this._c[2],this._c[3]));a.translate(10,a.height/2);a.stroke(e);a.strokeWeight(1);e=this._majorTicks*this._minorTicks;if(2<=e){d=c/e;for(var l=0;l<=e;l++){var m=l*d;a.line(m,-7,m,7)}}e=this._majorTicks;if(2<=e)for(d=c/this._majorTicks,
l=0;l<=e;l++)m=l*d,a.line(m,-10,m,10);a.fill(f);a.rect(0,-4,c,8);f=c*Math.min(this._t[0],this._t[1]);d=c*Math.max(this._t[0],this._t[1]);a.fill(g);a.rect(f,-4,d-f,8,this._c[0],this._c[1],this._c[2],this._c[3]);for(g=0;2>g;g++)a.fill(k),a.noStroke(),(this._active||0<this._over)&&g==this._tIdx&&(a.strokeWeight(2),a.stroke(h)),a.rect(this._t[g]*c-6,-6,12,12,this._c[0],this._c[1],this._c[2],this._c[3]);this._enabled||this._disable_hightlight(a,b,0,-this._h/2,this._w-20,this._h);a.pop();a.updatePixels();
this._bufferInvalid=!1}}Object.assign(CvsRanger.prototype,processMouse,processTouch);
...
(c.stroke(m),c.strokeWeight(2),c.noFill(),c.rect(1,1,this._w-2,this._h-2,this._c[0],this._c[1],this._c[2],this._c[3]));this._enabled||this._disable_hightlight(c,b,0,0,this._w,this._h);c.pop();c.updatePixels();this._bufferInvalid=!1;this._parent instanceof CvsPane&&this._parent.validateTabs()}_processEvent(a,...b){switch(a.type){case "mousedown":case "touchstart":0<this._over&&(this._active=this._dragging=this._clickAllowed=!0,this.invalidateBuffer());break;case "mouseout":case "mouseup":case "touchend":this._active&&
(this._clickAllowed&&this.action({source:this,p5Event:a}),this._over=0,this._active=this._dragging=this._clickAllowed=!1,this.invalidateBuffer());break;case "mousemove":case "touchmove":this._clickAllowed=!1}}}Object.assign(CvsButton.prototype,processMouse,processTouch);
...
this._buffer,b=this._scheme||this._gui.scheme(),c=Math.max(this._used*this._TLENGTH,this._MIN_THUMB_WIDTH),d=this._THUMB_HEIGHT,e=this._dvalue*this._TLENGTH;const f=b.C_3,g=b.G_8,h=b.G_3,k=b.C_9,l=b.C_5;a.push();a.clear();this._opaque&&(a.noStroke(),a.fill(f),a.rect(0,0,this._w,this._h,this._c[0],this._c[1],this._c[2],this._c[3]));a.translate(this._BORDER,a.height/2);a.fill(h);a.stroke(g);a.strokeWeight(1);a.rect(0,-this._THEIGHT/2,this._TLENGTH,this._THEIGHT);a.fill(l);a.noStroke();if(this._active||
0<this._over)a.strokeWeight(2),a.stroke(k);a.rect(e-c/2,-d/2,c,d,this._c[0],this._c[1],this._c[2],this._c[3]);this._enabled||this._disable_hightlight(a,b,0,-this._h/2,this._w-20,this._h);a.pop();a.updatePixels();this._bufferInvalid=!1}_minControlSize(){return{w:this._w,h:20}}}Object.assign(CvsScroller.prototype,processMouse,processTouch);
...
this._textSize||this._gui.textSize();let b=this._scheme||this._gui.scheme(),c=this._buffer;var d=this._iconAlign,e=this._p.constrain(.7*Number(a),12,16);let f=this._textAlign,g=this._lines,h=this._gap;var k=b.C_3;const l=b.C_8;var m=b.G_0;const n=b.G_9,p=b.C_9;c.push();c.clear();this._opaque&&(c.noStroke(),c.fill(k),c.rect(0,0,this._w,this._h,this._c[0],this._c[1],this._c[2],this._c[3]));c.push();c.translate(d==this._p.RIGHT?this._w-h-e/2:h+e/2,c.height/2);c.stroke(n);c.fill(m);c.strokeWeight(1.5);
c.rect(-e/2,-e/2,e,e,3);this._selected&&(c.strokeWeight(2.5),c.line(-.281*e,0,-.188*e,.313*e),c.line(.27*e,-.27*e,-.188*e,.313*e));c.pop();if(0<g.length){c.textSize(a);a=h;k=this._w-h;m=0;d==this._p.LEFT&&(a+=e+h);d==this._p.RIGHT&&(k-=e+h);d=k-a;e=this._tbox.h;e=c.textAscent()+(this._h-e)/2;c.fill(l);for(let q of g){switch(f){case this._p.LEFT:m=a;break;case this._p.CENTER:m=a+(d-c.textWidth(q))/2;break;case this._p.RIGHT:m=k-c.textWidth(q)-h}c.text(q,m,e);e+=c.textLeading()}}0<this._over&&(c.stroke(p),
c.strokeWeight(2),c.noFill(),c.rect(1,1,this._w-2,this._h-2,this._c[0],this._c[1],this._c[2],this._c[3]));this._enabled||this._disable_hightlight(c,b,0,0,this._w,this._h);c.pop();c.updatePixels();this._bufferInvalid=!1}_minControlSize(){let a=this._buffer,b=this._lines,c=this._tbox,d=0;var e=0;e=this._gap;var f=this._textSize||this._gui.textSize();f=this._p.constrain(.7*Number(f),12,16);if(0<b.length){a||this._validateBuffer();let g=this._textSize||this._gui.textSize();a.textSize(g);c.w=g+b.map(h=>
a.textWidth(h)).reduce((h,k)=>h>k?h:k);c.h=(b.length-1)*a.textLeading()+a.textAscent()+a.textDescent()}d+=c.w+e+f;e=Math.max(this._tbox.h,f+e)+2*e;return{w:d,h:e}}}Object.assign(CvsCheckbox.prototype,processMouse,processTouch);
...
(c=c.substring(0,this._currCsrIdx)+c.substring(this._currCsrIdx+1)),this._lines[0]=c}this.invalidateBuffer();return!1}return!0}_removeSelectedText(a){let b=Math.min(this._prevCsrIdx,this._currCsrIdx),c=Math.max(this._prevCsrIdx,this._currCsrIdx);this._prevCsrIdx=this._currCsrIdx=b;return a.substring(0,b)+a.substring(c)}_updateControlVisual(){var a=Number(this._textSize||this._gui.textSize());let b=this._scheme||this._gui.scheme(),c=this._buffer;c.textSize(a);let d=0<this._lines.length?this._lines[0]:
"";var e=this._textInvalid;let f=2*this._gap,g=b.C_1,h=b.C_9;const k=b.G_9,l=b.C_9,m=b.C_3;c.push();c.background(b.G_0);c.noStroke();if(!this._active)g=e?b.C_9:b.C_1,h=e?b.C_3:b.C_9,c.stroke(h),c.strokeWeight(1.5),c.fill(g),c.rect(0,0,this._w,this._h);else if(this._currCsrIdx!=this._prevCsrIdx){e=this._cursorX(c,d,this._prevCsrIdx);let n=this._cursorX(c,d,this._currCsrIdx);c.noStroke();c.fill(m);c.rect(f+Math.min(e,n),1,Math.abs(e-n),this._h-2)}c.fill(g);c.textSize(a);c.textAlign(this._p.LEFT,this._p.TOP);
c.noStroke();c.fill(h);c.text(d,f,(this._h-a)/2);this._activate&&this._cursorOn&&(a=this._cursorX(c,d,this._currCsrIdx),c.stroke(k),c.strokeWeight(1.5),c.line(f+a,4,f+a,this._h-5));0<this._over&&(c.stroke(l),c.strokeWeight(2),c.noFill(),c.rect(1,1,this._w-2,this._h-2,this._c[0],this._c[1],this._c[2],this._c[3]));this._enabled||this._disable_hightlight(c,b,0,0,this._w,this._h);c.pop();c.updatePixels();this._bufferInvalid=!1}}Object.assign(CvsTextField.prototype,processMouse,processTouch);
class CvsJoystick extends CvsBufferedControl{constructor(a,b,c,d,e,f){super(a,b,c||0,d||0,e||40,f||40);this._size=Math.min(e,f);this._pr0=.05*this._size;this._pr1=.4*this._size;this._tSize=Math.max(.075*this._size,6);this._mode="X0";this._ang=this._mag=0;this._opaque=!1;this._tmrID=void 0}mode(a){if(!a)return this._mode;a=a.toUpperCase();a;switch(a){case "X0":case "X4":case "X8":this._mode!=a&&(this._mode=a,this.invalidateBuffer())}return this}thumbSize(a){this._tSize=a;return this}_whereOver(a,b,
c=this._tSize){a-=this._w/2;b-=this._h/2;let [d,e]=this._getThumbXY();return Math.abs(d-a)<=c&&Math.abs(e-b)<=c?1:0}_getThumbXY(){return[this._mag*Math.cos(this._ang),this._mag*Math.sin(this._ang)]}_validateThumbPosition(a,b){let c=this._p.constrain(Math.sqrt(a*a+b*b),0,this._pr1);a=Math.atan2(b,a);a+=0>a?2*Math.PI:0;b=c<=this._pr0;let d=-1,e;switch(this._mode){case "X4":e=Math.PI/2;d=Math.floor((a+e/2)/e)%4;a=e*d;d*=2;break;case "X8":e=Math.PI/4,d=Math.floor((a+e/2)/e)%8,a=e*d}[this._mag,this._ang,
this._dir,this._dead]=[c,a,d,b]}_processEvent(a,...b){function c(e,f,g){return{source:e,p5Event:f,final:g,mag:(e._mag-e._pr0)/(e._pr1-e._pr0),angle:e._ang,dir:e._dir,dead:e._dead}}let d=b[0];b=b[1];d-=this._w/2;b-=this._h/2;switch(a.type){case "mousedown":case "touchstart":0<this._over&&(this._active=!0,this.invalidateBuffer());break;case "mouseout":case "mouseup":case "touchend":this._active&&(this._validateThumbPosition(d,b),this.action(c(this,a,!0)),this._active=!1,this.invalidateBuffer(),this._tmrID||
(this._tmrID=setInterval(()=>{this._mag-=.07*this._size;0>=this._mag&&(clearInterval(this._tmrID),this._tmrID=void 0,this._mag=0);this.invalidateBuffer()},25)));break;case "mousemove":case "touchmove":this._active&&(this._validateThumbPosition(d,b),this.action(c(this,a,!1)),this.invalidateBuffer())}}_updateControlVisual(){let a=this._buffer;var b=this._scheme||this._gui.scheme();let [c,d]=[this._mag*Math.cos(this._ang),this._mag*Math.sin(this._ang)];var e=b.C_3,f=b.C_1,g=b.T_0;const h=b.C_9,k=b.C_9,
l=b.C_4,m=b.C_6,n=b.C_7;var p=b.C_8;b=b.T_5;a.push();a.clear();this._opaque&&(a.noStroke(),a.fill(e),a.rect(0,0,this._w,this._h,this._c[0],this._c[1],this._c[2],this._c[3]));a.translate(a.width/2,a.height/2);a.noStroke();a.fill(f);a.ellipse(0,0,2*this._pr1,2*this._pr1);e=.26*this._size;a.fill(g);a.noStroke();a.ellipse(0,0,2*e,2*e);a.ellipse(0,0,1.25*e,1.25*e);a.stroke(p);switch(this._mode){case "X0":g=this._pr1;p=Math.PI/8;a.push();a.strokeWeight(.75);e=.3*this._size;for(f=0;16>f;f++)a.line(g,0,e,
0),a.rotate(p);a.pop();break;case "X8":g=this._pr0;e=.33*this._size;p=Math.PI/4;a.push();a.strokeWeight(1);for(f=0;8>f;f++)a.line(g,0,e,0),a.rotate(p);a.pop();case "X4":g=this._pr0;e=this._pr1;p=Math.PI/2;a.push();a.strokeWeight(1.5);for(f=0;4>f;f++)a.line(g,0,e,0),a.rotate(p);a.pop()}a.stroke(h);a.strokeWeight(Math.max(3,.025*this._size));a.noFill();a.ellipse(0,0,2*this._pr1,2*this._pr1);a.fill(b);a.noStroke();a.ellipse(0,0,2*this._pr0,2*this._pr0);a.stroke(n);a.strokeWeight(.05*this._size);a.line(0,
0,c,d);a.strokeWeight(2);a.stroke(k);this._active||0<this._over?a.fill(m):a.fill(l);a.ellipse(c,d,2*this._tSize,2*this._tSize);a.pop();a.updatePixels();this._bufferInvalid=!1}}Object.assign(CvsJoystick.prototype,processMouse,processTouch);
...
b,c=0){a-=this._w/2;b-=this._h/2;c=this._kRad+c;return a*a+b*b<=c*c?1:0}_updateControlVisual(){let a=this._buffer;var b=this._scheme||this._gui.scheme(),c=b.C_3,d=b.C_7;const e=b.C_8,f=b.C_3,g=b.C_9;var h=b.C_3,k=b.C_1,l=b.G_8;const m=b.G_2;b=b.T_1;a.clear();this._opaque&&(a.noStroke(),a.fill(c),a.rect(0,0,this._w,this._h,this._c[0],this._c[1],this._c[2],this._c[3]));c=this._turnArc;var n=(2*Math.PI-c)/2;let p=this._gRad;var q=2*this._kRad;let r=2*p;a.push();a.translate(a.width/2,a.height/2);a.rotate(this._gapPos+
n);a.noStroke();a.fill(h);a.ellipse(0,0,q,q);a.fill(k);a.arc(0,0,q,q,0,this._turnArc);h=this._majorTicks*this._minorTicks;if(2<=h){k=this._tw;n=.65*k;a.stroke(l);l=c/h;a.push();a.strokeWeight(.9);for(q=0;q<=h;q++)a.line(p,0,p+n,0),a.rotate(l);a.pop();h=this._majorTicks;if(2<=h){l=c/h;a.push();a.strokeWeight(1);for(n=0;n<=h;n++)a.line(p,0,p+k,0),a.rotate(l);a.pop()}a.noStroke();a.fill(b);a.arc(0,0,r+k,r+k,0,c);a.fill(m);a.arc(0,0,r+k,r+k,0,this._t01*c)}a.stroke(e);a.strokeWeight(1.5);a.fill(d);a.ellipse(0,
0,r,r);a.push();a.rotate(this._t01*c);d=.2*p;a.fill(f);a.noStroke();a.beginShape();a.vertex(-d,0);a.vertex(0,-d);a.vertex(p,0);a.vertex(0,d);a.endShape(this._p.CLOSE);a.pop();if(this._over||this._active)a.noFill(),a.stroke(g),a.strokeWeight(3),a.arc(0,0,2*this._kRad,2*this._kRad,0,c);a.pop();a.updatePixels();this._bufferInvalid=!1}_minControlSize(){return{w:this._size,h:this._size}}}CvsKnob.X_MODE=1;CvsKnob.Y_MODE=2;CvsKnob.A_MODE=3;Object.assign(CvsKnob.prototype,processMouse,processTouch);
