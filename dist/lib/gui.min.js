...
16777215^this._PART_MASK;this._NEXT_COLOR=this._COLOR_STEP;this._colorKey=new Map;this._ctrlKey=new Map;this._panesEast=[];this._panesSouth=[];this._panesWest=[];this._panesNorth=[];this._initColorSchemes();this._addFocusHandlers();this._addMouseEventHandlers();this._addTouchEventHandlers();this._drawHud=this._is3D?this._drawHudWEBGL:this._drawHudP2D;this._validateGuiBuffers();this._getCamera=1==Number(b.VERSION.split(".")[0])?function(){return this._renderer._curCamera}:function(){return this._renderer.states.curCamera}}_validateGuiBuffers(){let a=
...
typeof a||b||(this._schemes[a]?CERROR(`Cannot add scheme '${a}' because it already exists.'`):this._schemes[a]=b);return this}draw(){this._hud.clear();this._pickbuffer.clear();if(this._visible){this._p.push();for(let a of this._ctrls)a.getParent()||a._draw(this._hud,this._pickbuffer);this._p.pop();this._drawHud()}}_drawHudP2D(){this._visible&&(this._p.push(),this._p.image(this._hud,0,0),this._p.pop())}_drawHudWEBGL(){if(this._visible){this._p.push();let a=this._renderer,b=a.drawingContext,c=a.width,
d=a.height,e=Number.MAX_VALUE;b.flush();let f=a.uMVMatrix.copy(),h=a.uPMatrix.copy();b.disable(b.DEPTH_TEST);a.resetMatrix();this._getCamera().ortho(0,c,-d,0,-e,e);this._p.image(this._hud,0,0);b.flush();a.uMVMatrix.set(f);a.uPMatrix.set(h);b.enable(b.DEPTH_TEST);this._p.pop()}}getPicked(a,b){let c=this._pickbuffer,d={control:void 0,part:void 0};0<=a&&a<c.width&&0<=b&&b<c.height&&(a=c.get().get(a,b),a=(a[0]<<16)+(a[1]<<8)+a[2],d.control=this._colorKey.get(a&this._COLOR_MASK),d.part=a&this._PART_MASK);
return d}}const ANNOUNCE_CANVAS_GUI=function(){0==GUIS.size&&(CLOG("================================================"),CLOG(`  canvasGUI (${CANVAS_GUI_VERSION})   \u00A9 2025 Peter Lager`),CLOG("================================================"))},createGUI=function(a,b,c=p5.instance){ANNOUNCE_CANVAS_GUI();if(GUIS.has(a))return CWARN(`You already have a  GUI called '${a} it will not be replaced`),GUIS.get(a);b=new GUI(b,c);GUIS.set(a,b);return b},getGUI=function(a){return GUIS.get(a)};
