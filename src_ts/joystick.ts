...
class CvsJoystick extends CvsBufferedControl {
    ...
    /** @hidden */ protected _td: number;
    /** @hidden */ protected _ta: number;
    // Slices & rings
    /** @hidden */ protected _nSlices: number;
    /** @hidden */ protected _nRings: number;
    // Restore timer
     ...
     */
    decor(nslices: number, nrings: number) {
    ...
    _updateControlVisual(): void { // CvsStick
        ...
        b.noStroke(); b.fill(DIAL_FACE);
        b.ellipse(0, 0, this._pr1 * 2, this._pr1 * 2);
        // Dial face slices
        if (this._nSlices > 1) {
            let a = 2 * Math.PI / this._nSlices;
            b.push();
            b.stroke(DECOR); b.strokeWeight(1.5);
            for (let i = 0; i < this._nSlices; i++) {
                b.line(this._pr0, 0, this._pr1, 0); b.rotate(a);
            }
            b.pop();
        }
        // Dial radial ticks
        if (this._nRings > 1) {
            ...
            b.push();
            b.stroke(DECOR); b.strokeWeight(1.5); b.noFill();
        ...
        // Dial border
        b.stroke(DIAL_BORDER); b.strokeWeight(Math.max(3, 0.025 * this._size)); b.noFill();
        b.ellipse(0, 0, this._pr1 * 2, this._pr1 * 2);
        // Dead zone
        b.fill(DEAD_ZONE); b.noStroke();
        b.ellipse(0, 0, this._pr0 * 2, this._pr0 * 2);
        // Stick                                                                                    
        b.stroke(ROD); b.strokeWeight(this._size * 0.05);
        b.line(0, 0, tx, ty);
        // Thumb
        b.strokeWeight(2); b.stroke(THUMB_STROKE);
    ...
    _getValue() {
        ...
        let ta = this._ta; ta = ta >= 0 ? ta : ta + 2 * Math.PI;
    ...
    /** @hidden */
    _handleMouse(e: MouseEvent) { //    CvsJoystick
                    ...
                    if (!this._tmrID)
                        this._tmrID = setInterval(() => {
                            ...
                            if (this._td <= 0) {
                                ...
                                this._tmrID = undefined; this._td = 0;
                            }
                            this.invalidateBuffer();
                        }, 25);
                }
