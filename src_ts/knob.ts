...
class CvsKnob extends CvsSlider {
    ...
    /** @hidden */ static X_MODE = 1;
    /** @hidden */ static Y_MODE = 2;
    /** @hidden */ static A_MODE = 3;

    /** @hidden */ protected _size: number;
    // The arc size the knob can turn through
    /** @hidden */ protected _turnArc: number;
    // The orientation angle for midway between turn start and end positions
    /** @hidden */ protected _gapPos: number;
    // Pixel radii for active size
    /** @hidden */ protected _kRad: number;
    ...
    mode(mode: string) {
        switch (mode) {
            ...
            case 'a': this._mode = CvsKnob.A_MODE; break;
        }
        return this;
    }
    ...
    _tFromXY(x: number, y: number) {
        ...
        switch (this._mode) {
            case CvsKnob.X_MODE:
                ...
                under = t < 0; over = t > 1; t = constrain(t, 0, 1);
                break;
            case CvsKnob.Y_MODE:
    ...
    _processEvent(e: any, ...info) {
        let [mx, my] = info;
        mx -= this._w / 2; my -= this._h / 2;  // Make relative to knob centre
        switch (e.type) {
            case 'mousedown':
            case 'touchstart':
                if (this._over > 0) {
    ...
    _updateControlVisual(): void { // CvsSlider
        ...
        b.push();
        b.translate(b.width / 2, b.height / 2); b.rotate(this._gapPos + lowA);
        // Draw full background and track arc
        b.noStroke(); b.fill(TRACK_BACK); b.ellipse(0, 0, dOut, dOut);
        b.fill(TRACK_ARC); b.arc(0, 0, dOut, dOut, 0, this._turnArc);
        // Draw ticks? 
        ...
        if (n >= 2) {
            ...
            b.push(); {
                ...
                // minor ticks
                for (let i = 0; i <= n; i++) {
                    b.line(rIn, 0, rIn + b1, 0); b.rotate(da);
                }
            } b.pop();
            n = this._majorTicks;
            if (n >= 2) {
                ...
                b.push(); {
                    ...
                    // major ticks
                    for (let i = 0; i <= n; i++) {
                        b.line(rIn, 0, rIn + b0, 0); b.rotate(da);
                    }
                } b.pop();
            }
            // Unused track
            b.noStroke(); b.fill(UNUSED_TRACK);
        ...
        // Grip section
        b.stroke(GRIP_STROKE); b.strokeWeight(1.5); b.fill(GRIP_OFF);
        b.ellipse(0, 0, dIn, dIn);
        // Grip arrow marker
        b.push(); {
            b.rotate(this._t01 * arc);
            let ms = 0.2 * rIn;
            b.fill(MARKER); b.noStroke();
            b.beginShape();
            b.vertex(-ms, 0); b.vertex(0, -ms);
            b.vertex(rIn, 0); b.vertex(0, ms);
            b.endShape(this._p.CLOSE);
        } b.pop();
        // Is over highlight?
        if (this._over || this._active) {
            b.noFill(); b.stroke(HIGHLIGHT); b.strokeWeight(3);
