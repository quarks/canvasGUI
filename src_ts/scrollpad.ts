class CvsScrollpad extends CvsBufferedControl {
    ...
    /** @hidden */ protected _value: Array<number>;
    /** @hidden */ protected _dvalue: Array<number>;
    /** @hidden */ protected _used: Array<number>;

    /** @hidden */ protected _s_value: Array<number>;
    /** @hidden */ protected _s_mx: number;
    /** @hidden */ protected _s_my: number;

    /** @hidden */ protected _inset: number;
    /** @hidden */ protected _padW: number;
    /** @hidden */ protected _padH: number;

    /** @hidden */ protected _minThumbSize: number;

    /** @hidden */
    constructor(gui: GUI, name: string, x = 0, y = 0, w = 80, h = 80) {
    ...
    /** @hidden */
    _updateControlVisual() { // CvsScrollpad
    ...
    /** @hidden */
    _updateScrollerPickBuffer(inset: number, tx: number, ty: number, tw: number, th: number) {
    ...
    /** @hidden */
    isSameControl(ctrl: unknown): boolean {
        return this === ctrl;
    }
    ...
    /** @hidden */
    setValue(hValue = this._value[0], vValue = this._value[1]): void {
        function changeValue(sp: CvsScrollpad, idx: number, v: number) {
    ...
    /** @hidden */
    setUsed(hValue = this._used[0], vValue = this._used[1]): void {
     ...
     */
    actionInfo(event: any = undefined, final = false,) {
        return {
    ...
    _doEvent(e: MouseEvent | TouchEvent, x = 0, y = 0, over: any, enter: boolean): any {
        ...
        switch (e.type) {
            ...
            case 'touchmove':
                if (this.isActive) {
                    let newValueX: number | undefined = svalueH + (mx - this._s_mx) / padW;
                    newValueX = (newValueX + halfInset - halfUsedH >= 0 && newValueX - halfInset + halfUsedH <= 1)
                        ? newValueX : undefined;
                    let newValueY: number | undefined = svalueV + (my - this._s_my) / padH;
